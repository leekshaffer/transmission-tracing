...
p.1 <- ggplot(data=FullRes, aes(x=a*100, y=Ratio*100, group=k, color=k)) + 
  geom_line(size=0.5) +
  ...
  labs(x="Test Sensitivity (%)", y="Proportion of secondary cases isolated (%)") + 
  geom_abline(intercept=0, slope=1, col="grey70", size=0.5) + 
  facet_wrap(facets=vars(R0lab), nrow=2, ncol=2, labeller=label_parsed)
p.1

ggsave(filename="Fig1.png", plot=p.1, width=6, height=7, units="in", dpi=600)

p.2 <- ggplot(data=FullRes, aes(x=a*100, y=P.Iso*100, group=k, color=k)) + 
  geom_line(size=0.5) +
  scale_x_continuous(expand=c(0,0)) + scale_y_continuous(limit=c(0,100), expand=c(0,0)) + 
  scale_color_manual(name="Dispersion parameter (k)", values=colors) + 
  theme_bw() + theme(legend.position="top", panel.spacing = unit(1, "lines")) +
  labs(x="Test Sensitivity (%)", y="Proportion of index cases whose contacts are isolated (%)") + 
  facet_wrap(facets=vars(R0lab), nrow=2, ncol=2, labeller=label_parsed)
p.2

ggsave(filename="Fig2.png", plot=p.2, width=6, height=7, units="in", dpi=600)

p.c.1 <- ggplot(data=FullRes[!(FullRes$R0=="3"),], aes(x=a*100, y=Ratio*100, group=k, color=k)) + 
  geom_line(size=0.5) +
  scale_x_continuous(expand=c(0,0)) + scale_y_continuous(limit=c(0,100), expand=c(0,0)) +
  scale_color_manual(name="Dispersion parameter (k)", values=colors) + 
  theme_bw() + theme(legend.position="top", panel.spacing = unit(1, "lines")) +
...
p.c.2 <- ggplot(data=FullRes[!(FullRes$R0=="3"),], aes(x=a*100, y=P.Iso*100, group=k, color=k)) + 
  geom_line(size=0.5) +
  scale_x_continuous(expand=c(0,0)) + scale_y_continuous(limit=c(0,100), expand=c(0,0)) + 
  scale_color_manual(name="Dispersion parameter (k)", values=colors) + 
  theme_bw() + theme(legend.position="top", panel.spacing = unit(1, "lines")) +
